...
//----- 
   function ini_controle()
   {
      ...
      if ($this->NM_ajax_flag)
      {
          ...
          if (isset($this->NM_ajax_info['param']['salir']))
          {
              $this->salir = $this->NM_ajax_info['param']['salir'];
          }
...
?>
 ...
 <style>
 	...
 	.card{
 		background-color: #6cdb5e38 !important;
 	}
	 
	 .btn.btn-signin{
	 	background-color: rgb(57 213 62 / 42%) !important;
	 }
...
$_SESSION['scriptcase']['app_Login']['contr_erro'] = 'off';
      ...
      if (!$run_iframe && isset($_SESSION['scriptcase']['menu_atual']) && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['sc_outra_jan']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['sc_outra_jan']))
      {
          ...
          if ($this->Ini->sc_page == $this->sc_init_menu && !isset($_SESSION['scriptcase']['menu_apls'][$_SESSION['scriptcase']['menu_atual']][$this->sc_init_menu]['app_Login']))
          {
               ...
               $_SESSION['scriptcase']['menu_apls'][$_SESSION['scriptcase']['menu_atual']][$this->sc_init_menu]['app_Login']['label'] = "Sistema de Complementarias";
      ...
      $this->nm_new_label['pswd'] = '' . $this->Ini->Nm_lang['lang_sec_pasword_fild_login'] . '';
...
//
      if ($this->NM_ajax_flag && 'validate_' == substr($this->NM_ajax_opcao, 0, 9))
      {
          ...
          if ('validate_salir' == $this->NM_ajax_opcao)
          {
              $this->Valida_campos($Campos_Crit, $Campos_Falta, $Campos_Erros, 'salir');
          }
...
//
      ...
      if ($Campos_Crit != "" || !empty($Campos_Falta) || $this->Campos_Mens_erro != "" || $campos_erro != "" || !isset($this->bok) || $this->bok != "OK" || $this->nmgp_opcao == "recarga")
      {
          if ($Campos_Crit == "" && empty($Campos_Falta) && $this->Campos_Mens_erro == "" && !isset($this->bok) && $this->nmgp_opcao != "recarga")
          {
              ...
              { 
                  ...
                  $salir = $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['campos'][5]; 
              } 
          }
          $this->nm_gera_html();
          $this->NM_close_db(); 
      ...
      elseif (isset($this->bok) && $this->bok == "OK")
      {
          ...
          $_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['campos'][5] = $this->salir; 
...
<HEAD>
 <TITLE><?php echo strip_tags("Sistema de Complementarias") ?></TITLE>
   ...
   function Recupera_Nome_Campo($campo) 
   {
       switch($campo)
       {
           case 'login':
               ...
               break;
           case 'pswd':
               return "" . $this->Ini->Nm_lang['lang_sec_pasword_fild_login'] . "";
               break;
           case 'remember_me':
               return "";
               break;
...
//---------------------------------------------------------
     ...
     if (!is_array($filtro) && '' == $filtro && isset($this->nm_form_submit) && '1' == $this->nm_form_submit && $this->scCsrfGetToken() != $this->csrf_token)
     {
      ...
      if ((!is_array($filtro) && ('' == $filtro || 'salir' == $filtro)) || (is_array($filtro) && in_array('salir', $filtro)))
        $this->ValidateField_salir($Campos_Crit, $Campos_Falta, $Campos_Erros);
      if ((!isset($this->scFormFocusErrorName) || '' == $this->scFormFocusErrorName) && ( !empty($Campos_Crit) || !empty($Campos_Falta) ))
          $this->scFormFocusErrorName = "salir";
...
$_SESSION['scriptcase']['app_Login']['contr_erro'] = 'off'; 
    ...
    function ValidateField_pswd(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      { 
          ...
          { 
              $hasError = true;
              $Campos_Crit .= "" . $this->Ini->Nm_lang['lang_sec_pasword_fild_login'] . " " . $this->Ini->Nm_lang['lang_errm_mxch'] . " 32 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
          ...
          { 
              $hasError = true;
              $Campos_Crit .= "" . $this->Ini->Nm_lang['lang_sec_pasword_fild_login'] . " " . $this->Ini->Nm_lang['lang_errm_mnch'] . " 5 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
              if (!isset($Campos_Erros['pswd']))
              {
                  $Campos_Erros['pswd'] = array();
              }
    ...
    function ValidateField_salir(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
        global $teste_validade;
        $hasError = false;
      if ($this->nmgp_opcao != "excluir") 
      { 
          if (NM_utf8_strlen($this->salir) > 20) 
          { 
              $hasError = true;
              $Campos_Crit .= "Salir " . $this->Ini->Nm_lang['lang_errm_mxch'] . " 20 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
              if (!isset($Campos_Erros['salir']))
              {
                  $Campos_Erros['salir'] = array();
              }
              $Campos_Erros['salir'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 20 " . $this->Ini->Nm_lang['lang_errm_nchr'];
              if (!isset($this->NM_ajax_info['errList']['salir']) || !is_array($this->NM_ajax_info['errList']['salir']))
              {
                  $this->NM_ajax_info['errList']['salir'] = array();
              }
              $this->NM_ajax_info['errList']['salir'][] = $this->Ini->Nm_lang['lang_errm_mxch'] . " 20 " . $this->Ini->Nm_lang['lang_errm_nchr'];
          } 
      } 
        if ($hasError) {
            global $sc_seq_vert;
   ...
   function nm_guardar_campos()
   {
    ...
    $this->nmgp_dados_form['salir'] = $this->salir;
...
$_SESSION['sc_session'][$this->Ini->sc_page]['app_Login']['Lookup_remember_me'][] = '1';
   ...
   function ajax_return_values_retrieve_pswd($bForce = false)
   {
          ...
          //----- salir
   function ajax_return_values_salir($bForce = false)
   {
          if ('navigate_form' == $this->NM_ajax_opcao || 'backup_line' == $this->NM_ajax_opcao || (isset($this->nmgp_refresh_fields) && in_array("salir", $this->nmgp_refresh_fields)) || $bForce)
          {
              $sTmpValue = NM_charset_to_utf8($this->salir);
              $aLookup = array();
          $aLookupOrig = $aLookup;
          $this->NM_ajax_info['fldList']['salir'] = array(
                       'row'    => '',
               'type'    => 'text',
               'valList' => array($this->form_encode_input($sTmpValue)),
              );
...
<div class="scFormHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFormHeaderFont" style="float: left; text-transform: uppercase;"><?php if ($this->nmgp_opcao == "novo") { echo ""; } else { echo "Sistema de Complementarias"; } ?></div>
