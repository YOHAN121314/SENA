...
<HEAD>
 <TITLE><?php if ('novo' == $this->nmgp_opcao) { echo strip_tags(""); } else { echo strip_tags("Sistema de Complementarias"); } ?></TITLE>
...
<?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
    if (!isset($this->nm_new_label['salir']))
    {
        $this->nm_new_label['salir'] = "Salir";
    }
?>
<?php
   $nm_cor_fun_cel  = (isset($nm_cor_fun_cel) && $nm_cor_fun_cel  == $this->Ini->cor_grid_impar ? $this->Ini->cor_grid_par : $this->Ini->cor_grid_impar);
   ...
   $salir = $this->salir;
   $sStyleHidden_salir = '';
   if (isset($this->nmgp_cmp_hidden['salir']) && $this->nmgp_cmp_hidden['salir'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['salir']);
       $sStyleHidden_salir = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_salir = 'display: none;';
   $sStyleReadInp_salir = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['salir']) && $this->nmgp_cmp_readonly['salir'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['salir']);
       $sStyleReadLab_salir = '';
       $sStyleReadInp_salir = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['salir']) && $this->nmgp_cmp_hidden['salir'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="salir" value="<?php echo $this->form_encode_input($salir) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOdd css_salir_line" id="hidden_field_data_salir" style="<?php echo $sStyleHidden_salir; ?>"> 
<?php if ($bTestReadOnly && $this->nmgp_opcao != "novo" && isset($this->nmgp_cmp_readonly["salir"]) &&  $this->nmgp_cmp_readonly["salir"] == "on") { 

 ?>
<input type="hidden" name="salir" value="<?php echo $this->form_encode_input($salir) . "\">" . $salir . ""; ?>
<?php } else { ?>
<span id="id_read_on_salir" class="sc-ui-readonly-salir css_salir_line" style="<?php echo $sStyleReadLab_salir; ?>"><?php echo $this->form_format_readonly("salir", $this->form_encode_input($this->salir)); ?></span><span id="id_read_off_salir" class="css_read_off_salir<?php echo $this->classes_100perc_fields['span_input'] ?>" style="white-space: nowrap;<?php echo $sStyleReadInp_salir; ?>">
 <input class="sc-js-input scFormObjectOdd css_salir_obj<?php echo $this->classes_100perc_fields['input'] ?>" style="" id="id_sc_field_salir" type=text name="salir" value="<?php echo $this->form_encode_input($salir) ?>"
 <?php if ($this->classes_100perc_fields['keep_field_size']) { echo "size=10"; } ?> maxlength=20 alt="{datatype: 'text', maxLength: 20, allowedChars: '<?php echo $this->allowedCharsCharset("") ?>', lettersCase: '', enterTab: false, enterSubmit: true, autoTab: false, selectOnFocus: false, watermark: '', watermarkClass: 'scFormObjectOddWm', maskChars: '(){}[].,;:-+/ '}" ></span><?php } ?>
 </TD>
   <?php }?>
