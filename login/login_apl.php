...
//----- 
   function ini_controle()
   {
      ...
      if ($this->NM_ajax_flag)
      {
          ...
          if (isset($this->NM_ajax_info['param']['recordar_pswd']))
          {
              $this->recordar_pswd = $this->NM_ajax_info['param']['recordar_pswd'];
          }
          if (isset($this->NM_ajax_info['param']['recordar_pswd_sc_target_name']))
          {
              $this->recordar_pswd_sc_target_name = $this->NM_ajax_info['param']['recordar_pswd_sc_target_name'];
          }
          if (isset($this->NM_ajax_info['param']['remember_me']))
          {
              $this->remember_me = $this->NM_ajax_info['param']['remember_me'];
          }
          if (isset($this->NM_ajax_info['param']['remember_me_sc_target_name']))
          {
              $this->remember_me_sc_target_name = $this->NM_ajax_info['param']['remember_me_sc_target_name'];
          }
...
<style>
	...
	.card card-container{
		background-color: #4b6b4700;
		font-size: smaller
	}
	.page-sidebar page-background{
	}
	.page-sidebar page-background{
...
$_SESSION['scriptcase']['login']['contr_erro'] = 'off';
      ...
      if (!$run_iframe && isset($_SESSION['scriptcase']['menu_atual']) && (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['login']['sc_outra_jan']) || !$_SESSION['sc_session'][$this->Ini->sc_page]['login']['sc_outra_jan']))
      {
          ...
          if ($this->Ini->sc_page == $this->sc_init_menu && !isset($_SESSION['scriptcase']['menu_apls'][$_SESSION['scriptcase']['menu_atual']][$this->sc_init_menu]['login']))
          {
               ...
               $_SESSION['scriptcase']['menu_apls'][$_SESSION['scriptcase']['menu_atual']][$this->sc_init_menu]['login']['label'] = "login";
      ...
      $_SESSION['scriptcase']['error_icon']['login']  = "<img src=\"" . $this->Ini->path_icones . "/scriptcase__NM__btn__NM__scriptcase9_Lemon__NM__nm_scriptcase9_Lemon_error.png\" style=\"border-width: 0px\" align=\"top\">&nbsp;";
...
//
      if ($this->NM_ajax_flag && 'validate_' == substr($this->NM_ajax_opcao, 0, 9))
      {
          ...
          if ('validate_recordar_pswd' == $this->NM_ajax_opcao)
          {
              $this->Valida_campos($Campos_Crit, $Campos_Falta, $Campos_Erros, 'recordar_pswd');
          }
...
//
      ...
      if ($Campos_Crit != "" || !empty($Campos_Falta) || $this->Campos_Mens_erro != "" || $campos_erro != "" || !isset($this->bok) || $this->bok != "OK" || $this->nmgp_opcao == "recarga")
      {
          if ($Campos_Crit == "" && empty($Campos_Falta) && $this->Campos_Mens_erro == "" && !isset($this->bok) && $this->nmgp_opcao != "recarga")
          {
              ...
              { 
                  ...
                  $recordar_pswd = $_SESSION['sc_session'][$this->Ini->sc_page]['login']['campos'][3]; 
      ...
      elseif (isset($this->bok) && $this->bok == "OK")
      {
          ...
          $_SESSION['sc_session'][$this->Ini->sc_page]['login']['campos'][3] = $this->recordar_pswd; 
          $_SESSION['sc_session'][$this->Ini->sc_page]['login']['campos'][4] = $this->new_user; 
          if (!empty($this->remember_me))
          {
              $trab_saida = $this->remember_me;
              $diretorio = explode("/", $trab_saida);
              if (count($diretorio) > 2 || count($diretorio) == 0 || strtolower(substr($this->remember_me, 0, 7)) == "http://" || strtolower(substr($this->remember_me, 0, 8)) == "https://" || strtolower(substr($this->remember_me, 0, 3)) == "../")
              {
              ...
              else
              {
                  ...
                  $_SESSION['sc_session'][$this->Ini->sc_page]['login']['redir_target_name'] = $this->remember_me_sc_target_name;
...
//---------------------------------------------------------
     ...
     if (!is_array($filtro) && '' == $filtro && isset($this->nm_form_submit) && '1' == $this->nm_form_submit && $this->scCsrfGetToken() != $this->csrf_token)
     {
      ...
      if ((!is_array($filtro) && ('' == $filtro || 'recordar_pswd' == $filtro)) || (is_array($filtro) && in_array('recordar_pswd', $filtro)))
        $this->ValidateField_recordar_pswd($Campos_Crit, $Campos_Falta, $Campos_Erros);
      if ((!isset($this->scFormFocusErrorName) || '' == $this->scFormFocusErrorName) && ( !empty($Campos_Crit) || !empty($Campos_Falta) ))
          $this->scFormFocusErrorName = "recordar_pswd";
...
$_SESSION['scriptcase']['login']['contr_erro'] = 'off'; 
    ...
    function ValidateField_recordar_pswd(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
        global $teste_validade;
        $hasError = false;
      if ($this->nmgp_opcao != "excluir") 
      { 
          if (trim($this->recordar_pswd) != "")  
          { 
          } 
      } 
        if ($hasError) {
            global $sc_seq_vert;
            $fieldName = 'recordar_pswd';
            if (isset($sc_seq_vert) && '' != $sc_seq_vert) {
                $fieldName .= $sc_seq_vert;
            }
            $this->NM_ajax_info['fieldsWithErrors'][] = $fieldName;
        }
    } // ValidateField_recordar_pswd
   ...
   function nm_guardar_campos()
   {
    ...
    $this->nmgp_dados_form['recordar_pswd'] = $this->recordar_pswd;
...
//
   ...
   function ajax_return_values()
   {
          ...
          $this->ajax_return_values_recordar_pswd();
   ...
   function ajax_return_values_remember_me($bForce = false)
   {
          ...
          //----- recordar_pswd
   function ajax_return_values_recordar_pswd($bForce = false)
   {
          if ('navigate_form' == $this->NM_ajax_opcao || 'backup_line' == $this->NM_ajax_opcao || (isset($this->nmgp_refresh_fields) && in_array("recordar_pswd", $this->nmgp_refresh_fields)) || $bForce)
          {
              $sTmpValue = NM_charset_to_utf8($this->recordar_pswd);
              $aLookup = array();
          $aLookupOrig = $aLookup;
          $this->NM_ajax_info['fldList']['recordar_pswd'] = array(
                       'row'    => '',
               'type'    => 'text',
               'valList' => array($sTmpValue),
              );
...
$_SESSION['scriptcase']['login']['contr_erro'] = 'off'; 
    ...
    $recordar_pswd = $this->recordar_pswd;
...
<div class="scFormHeader" style="height: 54px; padding: 17px 15px; box-sizing: border-box;margin: -1px 0px 0px 0px;width: 100%;">
    <div class="scFormHeaderFont" style="float: left; text-transform: uppercase;"><?php if ($this->nmgp_opcao == "novo") { echo "" . $this->Ini->Nm_lang['lang_othr_frmi_titl'] . ""; } else { echo ""; } ?></div>
